...
void initMaze(char maze[ROW][COLUMN],
  ...
  *pPlayerPos = *pStartPos;
  // 0: 길 / 1: 벽 / 2: 입구 / 3: 출구
  strcpy(maze[0], "2100010000");
  strcpy(maze[1], "0101111110");
  strcpy(maze[2], "0000100000");
  strcpy(maze[3], "0110101011");
  strcpy(maze[4], "0010001003");
}
...
void movePlayer(char maze[ROW][COLUMN], Point* pPlayerPos, char ipt) {
  ...
  // 유효성 검사
  if (targetPos.y < 0 || targetPos.y >= ROW ||
      targetPos.x < 0 || targetPos.x >= COLUMN ||
